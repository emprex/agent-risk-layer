...
export const INSPECTION_TOKEN_TTL_MS = 15 * 60_000;
export const MAX_INSPECTION_AGE_MS = 24 * 60 * 60_000;
export const MAX_FINDINGS = 500;

export function createInspectionToken({ userId, assessmentId }) {
...
export function consumeInspectionUpload({ rawToken, bundle }) {
  if (!rawToken || !rawToken.startsWith('scan_')) throw new Error('Inspection token missing or invalid.');
  const tokenHash = hashToken(rawToken);
  const tokenRow = db.prepare(`
    SELECT * FROM inspection_tokens
    WHERE token_hash = ? AND used_at IS NULL AND expires_at > ?
  `).get(tokenHash, nowIso());
  if (!tokenRow) throw new Error('Inspection token is invalid, expired, or already used.');

  const validation = validateInspectionBundle(bundle);
  if (!validation.valid) throw new Error(`Evidence bundle rejected: ${validation.error}`);
  const createdAt = nowIso();
  ...
  const technologies = Array.isArray(bundle.observedTechnologies)
    ...
    : [];
  const previousRow = db.prepare('SELECT id, summary_json, findings_json FROM inspections WHERE assessment_id = ? ORDER BY created_at DESC LIMIT 1').get(tokenRow.assessment_id);
  ...
  try {
    ...
    if (claimed.changes !== 1) throw new Error('Inspection token is invalid, expired, or already used.');
    if (db.prepare('SELECT 1 AS ok FROM inspections WHERE bundle_digest = ?').get(validation.digest)) {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      inspectionId, tokenRow.user_id, tokenRow.assessment_id, bundle.schema,
      trust.scannerVersion, trust.policyVersion, validation.digest,
      JSON.stringify(subject), JSON.stringify(scope), JSON.stringify(summary),
      JSON.stringify(findings), JSON.stringify(technologies), JSON.stringify(trust), JSON.stringify(delta), createdAt,
    );
...
export function listInspectionsForAssessment({ assessmentId, userId }) {
  const owner = db.prepare('SELECT id FROM assessments WHERE id = ? AND user_id = ?').get(assessmentId, userId);
  if (!owner) throw new Error('Assessment not found.');
  return db.prepare(`
...
export function getInspection({ inspectionId, userId }) {
  const row = db.prepare('SELECT * FROM inspections WHERE id = ? AND user_id = ?').get(inspectionId, userId);
  return row ? publicInspection(row, true) : null;
}

export function latestInspection(assessmentId) {
  const row = db.prepare('SELECT * FROM inspections WHERE assessment_id = ? ORDER BY created_at DESC LIMIT 1').get(assessmentId);
  return row ? publicInspection(row, true) : null;
}
...
export function attachInspectionToResult(result, inspection) {
  ...
  if (observedCritical > 0) decision = 'DO NOT DEPLOY';
  else if (observedHigh > 0 && !String(decision).startsWith('DO NOT')) decision = 'DEPLOY ONLY AFTER MATERIAL REMEDIATION';
...
function buildInspectionDelta(previousRow, currentSummary, currentFindings) {
  if (!previousRow) return { status: 'first-scan', baselineInspectionId: null, newFindings: currentFindings.map(findingKey), resolvedFindings: [], unchangedCount: 0, technicalRiskChange: null, postureChange: null };
...
export function validateInspectionBundle(bundle) {
  try {
    ...
    if (bundle.schema !== INSPECTION_SCHEMA) return invalid('Unsupported schema.');
    if (!bundle.integrity || typeof bundle.integrity !== 'object') return invalid('Integrity block is missing.');
    if (!bundle.scanner || !/^3\./.test(String(bundle.scanner.version || ''))) return invalid('Unsupported scanner version.');
    const officialDigest = officialScannerDigest();
    if (!officialDigest || !safeEqual(officialDigest, String(bundle.scanner.buildDigest || ''))) return invalid('Reported scanner build digest does not match the published AgentRisk Inspector release.');
    if (!bundle.attestations?.authorisedByOperator) return invalid('Operator authorisation was not attested.');
    ...
    if (!Number.isFinite(generatedAt)) return invalid('Invalid generatedAt timestamp.');
    if (generatedAt > Date.now() + 5 * 60_000) return invalid('Bundle timestamp is in the future.');
    if (Date.now() - generatedAt > MAX_INSPECTION_AGE_MS) return invalid('Bundle is older than 24 hours. Generate a fresh inspection.');
    if (!Array.isArray(bundle.findings) || bundle.findings.length > MAX_FINDINGS) return invalid('Finding count exceeds the accepted limit.');
    if (containsForbiddenEvidence({ subject: bundle.subject, scope: bundle.scope, findings: bundle.findings, observedTechnologies: bundle.observedTechnologies, acceptedRiskReviews: bundle.acceptedRiskReviews, trust: bundle.trust })) return invalid('Evidence contains raw content or secret-like values.');
    const { integrity, ...payload } = bundle;
    const canonical = canonicalJson(payload);
    const digest = sha256(canonical);
    if (!safeEqual(digest, String(integrity.digest || ''))) return invalid('Digest mismatch.');
    if (integrity.signatureAlgorithm !== 'Ed25519') return invalid('Unsupported signature algorithm.');
    const publicKey = crypto.createPublicKey({
      key: Buffer.from(String(integrity.publicKeySpki || ''), 'base64'),
      type: 'spki', format: 'der',
    });
    const valid = crypto.verify(
      null, Buffer.from(canonical), publicKey,
      Buffer.from(String(integrity.signature || ''), 'base64'),
    );
...
function containsForbiddenEvidence(findings) {
  ...
  const visit = (value, depth = 0) => {
    ...
    if (typeof value === 'string') return value.length > 1000 || secretLike.test(value);
...
function normaliseValue(value, depth) {
  ...
  if (typeof value === 'object') return Object.fromEntries(Object.entries(value).slice(0, 50).map(([k,v]) => [clean(k,80), normaliseValue(v,depth+1)]));
  return null;
}
...
function officialScannerDigest() {
  try { return sha256(fs.readFileSync(path.resolve(process.cwd(), 'public', 'downloads', 'agent-risk-inspector.mjs'), 'utf8').replace(/\r\n/g, '\n')); }
  catch { return null; }
}
