...
async function writeApplicability({access,snapshot,controlId,userId,input}){
  ...
  if(!APPLICABILITY_DECISIONS.has(decision))throw badRequest('Decision must be applicable, not_applicable or context_required.');if(reason.length<10||/^(not relevant|not applicable|n\/a|none)$/i.test(reason))throw badRequest('A specific control-level applicability reason of at least 10 characters is required.');if(decision==='not_applicable'&&(!/\b(?:no|absent|disabled|cannot|never|without|does not|not configured)\b/i.test(reason)||!/\b(?:agent|system|capability|input|data|tool|access|authority|identity|network|boundary|action)\b/i.test(reason)))throw badRequest('Not-applicable reasons must identify the absent capability, data, authority, identity or trust boundary.');if(decision==='not_applicable'&&!facts.length)throw badRequest('Not-applicable decisions require at least one confirmed architecture fact.');const snapshotFactsSet=new Set(snapshotFacts(snapshot));if(facts.some(fact=>!ARCHITECTURE_FACTS.includes(fact)||!snapshotFactsSet.has(fact)))throw badRequest('Architecture facts must be confirmed facts from the current snapshot.');
