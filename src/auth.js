...
export async function registerUser(email, password, termsAccepted = false, inviteCode = '') {
  ...
  const invite = config.requireBetaInvite && normalized !== config.adminEmail ? validateInvite(inviteCode, normalized) : null;
  ...
  try {
    ...
    try {
      db.prepare(`INSERT INTO users (id, email, password_hash, email_verified_at, mfa_enabled_at, role, terms_version, terms_accepted_at, created_at)
        VALUES (?, ?, ?, NULL, NULL, ?, ?, ?, ?)`)
        .run(user.id, user.email, passwordHash, user.role, user.terms_version, user.terms_accepted_at, user.created_at);
      if (invite) {
        const used = db.prepare(`UPDATE beta_invites SET status='used',used_by=?,used_at=?
          WHERE id=? AND status='active' AND used_at IS NULL`).run(user.id, created, invite.id);
        if (Number(used.changes) !== 1) throw new Error('This beta invitation has already been used.');
      }
      db.exec('COMMIT');
...
function validateInvite(code, email) {
  ...
  if (!value) throw new Error('A beta invitation code is required.');
  const codeHash = tokenHash(value, 'beta-invite');
  const invite = db.prepare(`SELECT * FROM beta_invites WHERE code_hash=? AND status='active' AND used_at IS NULL
    AND (expires_at IS NULL OR expires_at>?)`).get(codeHash, nowIso());
  if (!invite || (invite.email && invite.email !== email)) throw new Error('This beta invitation is invalid, expired or assigned to another email.');
  return invite;
}
