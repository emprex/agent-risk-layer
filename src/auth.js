...
export async function verifyPassword(password, stored) {
    try {
        ...
        if (algorithm !== 'scrypt' || !salt64 || !key64)
            return false;
...
export async function createSession(res, userId, { mfaVerified = false } = {}) {
    const token = b64(crypto.randomBytes(32));
    const now = nowIso();
    const expires = new Date(Date.now() + config.sessionAbsoluteDays * 86400000);
    await db.prepare('DELETE FROM sessions WHERE expires_at <= ? OR last_seen_at <= ?')
  ...
  `).run(tokenHash(token), userId, expires.toISOString(), now, now, now, mfaVerified ? 1 : 0);
    const sessions = await db.prepare('SELECT token_hash FROM sessions WHERE user_id = ? ORDER BY created_at DESC').all(userId);
    for (const old of sessions.slice(10))
        await db.prepare('DELETE FROM sessions WHERE token_hash = ?').run(old.token_hash);
    const secure = config.nodeEnv === 'production' ? '; Secure' : '';
    appendSetCookie(res, `${COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${config.sessionAbsoluteDays * 86400}${secure}`);
}
...
export async function getUserFromRequest(req) {
    ...
    SELECT users.id, users.email, users.role, users.email_verified_at, users.mfa_enabled_at,
  ...
  `).get(tokenHash(token), now, idleCutoff);
    ...
    if (Date.now() - Date.parse(row.last_seen_at) > 5 * 60000) {
        await db.prepare('UPDATE sessions SET last_seen_at = ? WHERE token_hash = ?').run(now, row.session_token_hash);
...
export async function beginMfaSetup(userId, password) {
    ...
    const user = await db.prepare('SELECT email,mfa_enabled_at FROM users WHERE id=?').get(userId);
    if (!user)
        throw new Error('Account not found.');
    if (user.mfa_enabled_at)
        throw new Error('Multi-factor authentication is already enabled.');
...
export async function enableMfa(userId, { password, secret, code }) {
    ...
    if (!verifyTotp(cleanSecret, code))
        throw new Error('The authentication code is invalid.');
...
export async function disableMfa(userId, { password, code }) {
    if (!await verifyUserPassword(userId, password))
        throw new Error('Password is incorrect.');
    const row = await db.prepare('SELECT mfa_secret_encrypted,mfa_recovery_codes_json FROM users WHERE id=?').get(userId);
...
export async function reauthenticateSession(req, password, code = '') {
    ...
    if (!user || !await verifyUserPassword(user.id, password))
        throw new Error('Password is incorrect.');
    if (user.mfaEnabled) {
        const row = await db.prepare('SELECT mfa_secret_encrypted,mfa_recovery_codes_json FROM users WHERE id=?').get(user.id);
...
export function publicUser(row) {
    return {
        id: row.id,
        email: row.email,
        role: row.role || (row.email === config.adminEmail ? 'superuser' : 'user'),
        isSuperuser: (row.role || (row.email === config.adminEmail ? 'superuser' : 'user')) === 'superuser',
...
async function verifyMfaCode(userId, encryptedSecret, recoveryJson, code) {
    ...
    if (/^\d{6}$/.test(value) && encryptedSecret && verifyTotp(decrypt(encryptedSecret), value))
        return true;
...
function verifyTotp(secret, code, now = Date.now()) {
    try {
        ...
        for (let offset = -1; offset <= 1; offset += 1) {
            ...
            if (safeEqual(expected, String(code)))
                return true;
        }
    }
    catch { }
