...
export function buildReport(assessment,tier='basic',inspection=null,redTeam=null){
  ...
  if((inspection?.summary?.counts?.critical||0)>0||(redTeamIsTarget&&redTeam?.summary?.counts?.critical>0))combinedDecision='DO NOT DEPLOY';
  else if((inspection?.summary?.counts?.high||0)>0||(redTeamIsTarget&&redTeam?.summary?.counts?.high>0))combinedDecision='DEPLOY ONLY AFTER MATERIAL REMEDIATION';
  else if(redTeamIsTarget&&(redTeam?.summary?.counts?.failed||0)>0&&!String(combinedDecision).startsWith('DO NOT'))combinedDecision='REMEDIATE BEFORE RELEASE';
