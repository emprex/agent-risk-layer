...
const script = read('public/control-intelligence-scope-review.js');
const css = read('public/control-intelligence-scope-review.css');

test('Deployment Evidence loads the compact scope-review layer after the existing Control Intelligence UI', () => {
  assert.match(html, /control-intelligence-scope-review\.css/);
  assert.match(html, /control-intelligence-scope-review\.js/);
  assert.ok(html.indexOf('control-intelligence-customer-guidance.js') < html.indexOf('control-intelligence-scope-review.js'));
});
...
test('prepared rows use only existing form fields and confirmed snapshot facts', () => {
...
test('render observation is bounded to root child replacement and cannot self-loop on subtree edits', () => {
