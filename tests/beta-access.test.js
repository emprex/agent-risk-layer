...
test('configured owner is a durable superuser without consuming a beta invitation', async () => {
    ...
    assert.equal((await db.prepare('SELECT COUNT(*) count FROM beta_invites').get()).count, 0);
});
test('beta invitation is email-bound and single-use', async () => {
    ...
    await db.prepare(`INSERT INTO beta_invites (id,code_hash,email,status,created_at,expires_at)
    VALUES (?,?,?,'active',?,?)`).run(id('inv_'), codeHash, 'beta@example.com', nowIso(), new Date(Date.now() + 86400000).toISOString());
    await assert.rejects(async () => await registerUser('wrong@example.com', 'Strong-password-42', true, code), /invalid, expired or assigned/);
    const user = await registerUser('beta@example.com', 'Strong-password-42', true, code);
    assert.equal(user.role, 'user');
    assert.equal((await db.prepare('SELECT status FROM beta_invites WHERE code_hash=?').get(codeHash)).status, 'used');
    await assert.rejects(async () => await registerUser('beta2@example.com', 'Strong-password-42', true, code), /invalid, expired or assigned/);
});
