...
test.before(async () => {
    ...
    userId = user.id;
    await db.prepare('UPDATE users SET email_verified_at=? WHERE id=?').run(nowIso(), userId);
...
test('password verification uses asynchronous scrypt without monopolising the event loop', async () => {
    ...
    const outcomes = await Promise.all(Array.from({ length: 6 }, async () => await verifyPassword('Strong-password-42', hash)));
    clearInterval(timer);
...
test('email-verified accounts can enable TOTP MFA and complete a protected sign-in', async () => {
    ...
    const challenge = await createMfaLoginChallenge(userId);
    assert.equal(await completeMfaLogin(challenge.challengeToken, totp(setup.secret)), userId);
});
test('primary rate policy keeps health and public reads available during bounded bursts', async () => {
    ...
    for (let i = 0; i < 600; i += 1)
        assert.equal(await primaryRateLimitAllowed(healthReq, '/api/health'), true);
    for (let i = 0; i < 300; i += 1)
        assert.equal(await primaryRateLimitAllowed(publicReq, '/'), true);
});
test('trusted-proxy rate limiting cannot be bypassed by changing the left-most forwarded address', async () => {
    ...
    assert.equal(await rateLimitAllowed(reqA, { bucket: 'spoof-test', windowMs: 60000, max: 1 }), true);
    assert.equal(await rateLimitAllowed(reqB, { bucket: 'spoof-test', windowMs: 60000, max: 1 }), false);
});
test('expired red-team evidence is deleted with a receipt while legal hold is preserved', async () => {
    const now = nowIso();
    const authId = id('roe_');
    await db.prepare(`INSERT INTO redteam_authorisations (id,user_id,assessment_id,target_name,endpoint_origin,environment,authority_basis,authorised_by,authorised_role,emergency_contact,window_start,window_end,permitted_actions_json,prohibited_actions_json,data_classification,retention_days,synthetic_data_only,dry_run_tools_only,status,attestation_text,accepted_at,revoked_at,created_at,legal_hold)
    ...
    assert.equal((await db.prepare('SELECT COUNT(*) count FROM redteam_runs WHERE authorisation_id=?').get(authId)).count, 0);
    assert.equal((await db.prepare('SELECT COUNT(*) count FROM data_purge_receipts WHERE authorisation_id=?').get(authId)).count, 1);
    ...
    assert.equal((await db.prepare('SELECT COUNT(*) count FROM redteam_runs WHERE authorisation_id=?').get(holdId)).count, 1);
});
