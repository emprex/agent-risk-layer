...
test('public Inspector 4.1.5 does not treat unrelated schema validation as agent output evidence', async (t) => {
  ...
  assert.equal(INSPECTOR_VERSION, '4.1.5');
  assert.ok(bundle.findings.some((finding) => finding.ruleId === 'ARL-AI-006'));
});

test('public Inspector 4.1.5 recognises validation at the AI integration boundary', async (t) => {
  const directory = temporaryRepository(t);
