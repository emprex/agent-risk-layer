...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/admin/invites') {
      if (!requireAdmin(req, res, { requireMfa: true })) return;
      return json(res, 200, betaInviteSummary());
    }

    if (req.method === 'POST' && url.pathname === '/api/admin/invites') {
      if (!requireAdmin(req, res, { requireMfa: true })) return;
      const body = await readBody(req);
      try { return json(res, 201, createBetaInvite(req.user, body)); }
      catch (error) { return json(res, 400, { error: error.message }); }
    }
    ...
    if (req.method === 'POST' && match) {
      if (!requireAdmin(req, res, { requireMfa: true })) return;
      const changed = db.prepare(`UPDATE beta_invites SET status='revoked',revoked_at=? WHERE id=? AND status='active'`).run(nowIso(), decodeURIComponent(match[1]));
      if (Number(changed.changes) !== 1) return json(res, 404, { error: 'Active invitation not found.' });
      insertEvent('beta_invite_revoked', req.user.id, { inviteId: decodeURIComponent(match[1]) });
      return json(res, 200, { ok: true });
    }
...
function requireAdmin(req, res, { requireMfa = false } = {}) {
  ...
  if (!req.user.isSuperuser) { json(res, 403, { error: 'Superuser access required.' }); return false; }
...
function betaInviteSummary() {
  const invites = db.prepare(`SELECT id,email,status,created_at,expires_at,used_at,revoked_at FROM beta_invites ORDER BY created_at DESC`).all();
  return {
...
function createBetaInvite(user, input = {}) {
  const summary = betaInviteSummary();
  if (summary.remaining <= 0) throw new Error(`The ${config.betaInviteLimit}-invite controlled-beta capacity has been reached.`);
  const email = cleanText(input.email || '', 254).toLowerCase() || null;
  if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error('Enter a valid invitee email or leave it blank.');
  ...
  db.prepare(`INSERT INTO beta_invites (id,code_hash,email,status,created_by,created_at,expires_at) VALUES (?,?,?,'active',?,?,?)`)
    .run(inviteId, codeHash, email, user.id, nowIso(), expiresAt);
  insertEvent('beta_invite_created', user.id, { inviteId, assigned: Boolean(email), expiresAt });
...
function renderSitemap() {
  const paths = ['/', '/assessment.html', '/pricing.html', '/methodology.html', '/help.html', '/sample-report.html', '/trust.html', '/redteam.html', '/privacy.html', '/terms.html', ...Object.keys(seoPages).map((slug) => `/checks/${slug}`)];
