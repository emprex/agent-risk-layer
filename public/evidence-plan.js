const BOUNDED_CHECKS = Object.freeze([
  {
    id: 'mcp-authority',
    match: /tool|mcp|authori[sz]|permission|allowlist/i,
  ...
  {
    id: 'approval-binding',
    match: /approval|human oversight|high-impact action/i,
  ...
  {
    id: 'memory-isolation',
    match: /memory|tenant|session|context isolation/i,
  ...
  {
    id: 'egress-boundary',
    match: /egress|outbound|network/i,
  ...
  },
  {
    ...
    match: /kill switch|contain|recovery|incident response|revocation/i,
    ...
    cases: ['Stop execution', 'Revoke credential', 'Block tool access', 'Preserve evidence', 'Safe-state restart'],
    environment: 'non-production',
  },
  {
    id: 'audit-reconstruction',
    match: /logging|observability|audit|reconstruct/i,
    ...
    cases: ['Identity recorded', 'Action recorded', 'Approval / policy recorded', 'Outcome recorded', 'Correlation retained'],
    environment: 'non-production',
  },
]);
...
export function buildEvidencePlan({ assessment = {}, inspections = [] } = {}) {
  ...
  if (!latestInspection) {
    return {
      ...
      title: 'Run source evidence first',
      explanation: 'AgentRiskLayer needs observed source evidence before it selects runtime checks. A declaration is not proof, and runtime tests should be limited to questions source review cannot resolve.',
      checks: [],
      manual: gaps,
    };
  }
  ...
  return {
    state: gaps.length ? 'manual-evidence-required' : 'no-runtime-check-selected',
    title: gaps.length ? 'No bounded runtime check is justified automatically' : 'No material evidence gap requires a bounded runtime check',
    explanation: gaps.length
      ? 'The remaining evidence questions are not mapped to a safe bounded runtime invariant. Keep them as evidence gaps until a reviewer defines an appropriate test; do not invent a finding or run a generic attack suite.'
      : 'Current assessment data does not identify a material evidence gap that needs runtime verification. This is not a deployment approval.',
    checks: [],
    manual,
  };
}

export function evidencePlanCatalog() {
  return BOUNDED_CHECKS.map(({ match, ...item }) => ({ ...item }));
}
