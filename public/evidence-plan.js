const BOUNDED_CHECKS = Object.freeze([
  { id:'mcp-authority', match:/tool|mcp|authori[sz]|permission|allowlist/i, title:'Verify tool / MCP authority enforcement', why:'Source review can show that a policy exists, but only a bounded runtime check can show whether an unauthorised agent or context is actually denied at execution time.', invariant:'A tool or MCP action outside the active agent authority must be denied before execution.', cases:['Deny','Allow','Isolation','Plugin / tool scope','Fail closed'], environment:'non-production', caseId:'RT-AUTH-001' },
  { id:'approval-binding', match:/approval|human oversight|high-impact action/i, title:'Verify exact-action approval binding', why:'A declared approval flow is not proof that approval is bound to the exact action, target and parameters at execution time.', invariant:'A consequential action must execute only with a valid approval bound to that exact action and parameters.', cases:['No approval denied','Exact approval allowed','Changed parameters denied','Expired approval denied','Replay denied'], environment:'non-production', caseId:'RT-PI-008' },
  { id:'memory-isolation', match:/memory|tenant|session|context isolation/i, title:'Verify memory / tenant isolation', why:'Static configuration can indicate intended separation, but bounded execution is needed to show that one user or tenant cannot read another context.', invariant:'Data written under one authorised identity must not be retrievable by a different unauthorised identity.', cases:['Owner read allowed','Cross-user read denied','Cross-tenant read denied','Namespace mismatch denied','Fail closed'], environment:'non-production', caseId:'RT-MEM-002' },
  { id:'egress-boundary', match:/egress|outbound|network/i, title:'Verify outbound network boundary', why:'Configuration alone does not prove that blocked destinations cannot be reached by the running agent.', invariant:'A destination outside the approved outbound policy must be blocked before data leaves the controlled environment.', cases:['Allowed destination works','Blocked destination denied','Redirect does not bypass','Alternate protocol denied','Fail closed'], environment:'non-production', caseId:'RT-TOOL-004' },
