...
function checkHtml(check, index, assessmentId) {
  return `<article class="finding-work-item" ${index === 0 ? 'data-primary-evidence-check="true"' : ''}><div class="finding-work-body">
    ...
    <div class="button-row compact">${check.caseId ? `<a class="button primary small" href="${checkAction(check, assessmentId)}">Open bounded check</a>` : ''}<button class="button secondary small" type="button" data-evidence-not-applicable="${escapeHtml(check.id)}">Mark not applicable</button></div>
    <p class="microcopy">Use Not applicable only when reviewed evidence shows this runtime boundary is not materially present. This is not a PASS or verified control.</p>
  </div></article>`;
}

function resolvedHtml(item) { const r = item.resolution || {}; return `<article class="finding-work-item"><div class="finding-work-body"><div class="question-meta"><span>Resolved by applicability</span><span>Not applicable</span></div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(r.rationale || 'No material runtime boundary identified.')}</p><p class="microcopy">This resolution is not a PASS, verified control or deployment approval.</p></div></article>`; }

function planHtml(plan, assessmentId) {
  const resolved = plan.resolved?.length ? `<details class="workspace-technical section-gap"><summary><span>${plan.resolved.length} applicability resolution${plan.resolved.length === 1 ? '' : 's'}</span><small>Not counted as PASS</small></summary><div class="workspace-technical-body"><div class="plain-finding-list">${plan.resolved.map(resolvedHtml).join('')}</div></div></details>` : '';
  ...
  if (plan.state === 'bounded-check-required') return `<section class="workspace-section section-gap" data-evidence-plan><div class="workspace-section-heading"><div><span class="eyebrow">Evidence plan</span><h2>${escapeHtml(plan.checks.length === 1 ? '1 bounded runtime check selected' : `${plan.checks.length} bounded runtime checks selected`)}</h2><p>${escapeHtml(plan.explanation)}</p></div></div><div class="success-box"><strong>Source evidence complete.</strong><p>Run only the bounded checks needed for material questions source review cannot prove.</p></div><div class="plain-finding-list">${plan.checks.map((c,i)=>checkHtml(c,i,assessmentId)).join('')}</div>${resolved}${plan.manual.length ? `<details class="workspace-technical section-gap"><summary><span>${plan.manual.length} other evidence question${plan.manual.length===1?'':'s'}</span></summary><div class="workspace-technical-body"><ul class="check-list">${plan.manual.map(g=>`<li>${escapeHtml(gapLabel(g))}</li>`).join('')}</ul></div></details>` : ''}</section>`;
  return `<section class="workspace-section section-gap" data-evidence-plan><span class="eyebrow">Evidence plan</span><h2>${escapeHtml(plan.title)}</h2><p>${escapeHtml(plan.explanation)}</p>${resolved}${plan.manual.length ? `<ul class="check-list">${plan.manual.map(g=>`<li>${escapeHtml(gapLabel(g))}</li>`).join('')}</ul>` : ''}</section>`;
}
...
document.addEventListener('click',async e=>{
  ...
  const button=e.target?.closest?.('[data-evidence-not-applicable]'); if(!button)return;
  const assessmentId=selectedAssessmentId(); const planId=button.dataset.evidenceNotApplicable; if(!assessmentId||!planId)return;
  const rationale=window.prompt('Why is this bounded runtime check not materially applicable? Record the evidence-based reason.');
  ...
  try { await api(`/api/assessments/${encodeURIComponent(assessmentId)}/evidence-plan/resolutions`,{method:'POST',body:JSON.stringify({planId,state:'not-applicable',rationale:rationale.trim()})}); activeAssessmentId=''; await loadPlan(assessmentId); }
