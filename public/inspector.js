...
function render(){
  ...
  root.innerHTML=`
    <section class="workspace-agent-command">
      ...
      ${selected?`<div class="workspace-status-grid"><div class="workspace-status-card" data-state="unresolved"><small>Evidence class</small><strong>Observed</strong><p>Inspector results remain separate from declared assessment answers.</p></div><div class="workspace-status-card" data-state="unresolved"><small>Trust boundary</small><strong>Read-only source inspection</strong><p>GitHub source is inspected server-side at the frozen commit. Local inspection remains available when source must stay on your machine.</p></div><div class="workspace-next-action"><small>Next action</small><strong>Run the authorised Inspector against the exact system version.</strong><p>The one-time upload token expires and can be used once.</p><button class="button primary small" id="createToken">Create inspection command</button></div></div>`:''}
    </section>
