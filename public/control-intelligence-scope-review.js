...
function decorateControls() {
  ...
  guide.innerHTML = `
    <div>
      <span class="eyebrow">Customer scope review</span>
      <h3>Review exceptions, not 108 controls one by one</h3>
      <p>Strong architecture matches can be prepared as <strong>suggested applicability choices</strong> from facts you already confirmed in the immutable snapshot. Nothing is saved until you review and confirm the batch.</p>
    </div>
    <div class="ci-scope-principles" aria-label="Scope review principles">
      <span>Matched ≠ failed</span>
      <span>Unknown ≠ finding</span>
      <span>No automatic deployment decision</span>
    </div>`;
  ...
  panel.querySelectorAll('.ci-control-row').forEach((row) => {
    ...
    if (title && !row.querySelector('.ci-security-question-label')) {
      ...
      label.className = 'ci-security-question-label';
      label.textContent = 'Security question';
...
function setRowState(fieldset, meta) {
  ...
  summaryCard.innerHTML = `
    <div>
      <span class="ci-scope-badge ${strong ? 'prepared' : 'review'}">${strong ? 'Prepared suggestion · Applicable' : 'Needs your review'}</span>
      <strong>${meta?.title || controlId}</strong>
      <p>${meta?.rationale || 'Review this security question against the confirmed system facts.'}</p>
    </div>
    <span class="ci-scope-facts">${meta?.factText || (facts.length ? `${facts.length} snapshot fact${facts.length === 1 ? '' : 's'}` : 'No deterministic fact match')}</span>`;
  ...
  summary.textContent = strong ? 'Review or change this prepared decision' : 'Complete this decision';
...
function compactBulkReview() {
  ...
  summary.innerHTML = `
    <div><strong>${rows.length}</strong><span>selected controls</span></div>
    <div><strong>${prepared}</strong><span>prepared strong matches</span></div>
    <div><strong>${exceptions}</strong><span>need manual review</span></div>
    <p>Saving creates individual snapshot-bound applicability decisions. It does not create test results, findings, remediation records or a deployment decision.</p>`;
