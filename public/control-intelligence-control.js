...
function testForm() {
  ...
  const task = assessmentFixControl(handoff.findingId);
  const developerPack = task && task.controlId === controlId ? `<section class="ci-developer-pack"><span class="eyebrow">Developer test pack · ${esc(task.controlId)}</span><h3>Give this to the person connecting the staging agent</h3><ol><li><strong>Implement:</strong> ${esc(task.outcome)}</li><li><strong>Run:</strong> ${esc(task.test)}</li><li><strong>Return:</strong> ${esc(task.proof)}</li><li>Bind the result to this exact snapshot and remove secrets and unnecessary customer data before upload.</li></ol><p><strong>Pass:</strong> the expected safe result is demonstrated by the matching trace or record. <strong>Fail:</strong> the unsafe condition reproduces, the expected control does not act, or the required proof cannot be reconstructed.</p><button class="button ghost small" id="copyDeveloperTestPack" type="button">Copy developer test pack</button></section>` : '';
...
function wire() {
  document.querySelector('#copyDeveloperTestPack')?.addEventListener('click', async () => {
    const task = assessmentFixControl(handoff.findingId);
    if (!task || task.controlId !== controlId) return;
    const text = `${handoff.findingId} developer test pack — ${task.controlId}\nImplement: ${task.outcome}\nRun: ${task.test}\nReturn: ${task.proof}\nBind the result to this exact snapshot and remove secrets and unnecessary customer data before upload.\nPass: the expected safe result is demonstrated by the matching trace or record.\nFail: the unsafe condition reproduces, the expected control does not act, or the required proof cannot be reconstructed.`;
