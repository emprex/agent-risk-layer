...
try{
  ...
  assert.equal(migration.migrations.applied.length,16);assert.equal(migration.migrations.applied.at(-1),'016_control_intelligence_journey.sql');
  assert.equal(Number((await db.prepare('SELECT COUNT(*) count FROM schema_migrations').get()).count),16);
  const {runMigrations}=await import('../src/migrations.js');const repeated=await runMigrations(db);assert.equal(repeated.applied.length,0);assert.equal(repeated.skipped.length,16);
