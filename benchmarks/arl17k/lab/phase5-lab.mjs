...
const SYNTHETIC_CONTROL_TOKEN = 'ARL17K_SYNTHETIC_CONTROL_TOKEN_V3';

export class Phase5Lab {
  #observer;
  #mode;
  #deniedPathThreshold;
  #deniedPathCount = 0;
  ...
  constructor({ observer, mode, deniedPathThreshold = 25, successRoute = 'mock_control_plane' }) {
    if (!['unsafe_baseline', 'remediated'].includes(mode)) {
      throw new Error('mode must be unsafe_baseline or remediated.');
    }
    ...
    this.#runtimePolicy = compileRuntimePolicy({
      version: 'arl17k-phase5-runtime-v1',
      mode: 'enforce',
    });
  }
  ...
  attemptBoundary({ attempt, route }) {
    if (this.#mode === 'remediated' && this.#breakerOpen) {
    ...
    if (this.#mode === 'remediated' && outcome === 'denied') {
